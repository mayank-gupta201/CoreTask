...
// Feature 6: Subtask list component
const SubtaskList = ({ parentId }: { parentId: string, members?: WorkspaceMember[] }) => {
...
export function TaskList() {
    ...
    // Feature 5: Activities with pagination
    const {
        data: activitiesData,
            ...
            <Sheet open={isSheetOpen} onClose={() => setSheetOpen(false)}>
                <div className="flex flex-col h-full bg-background">
                    <div className="flex-1 overflow-y-auto">
                        <div className="px-6 py-5 space-y-5">
                            <div className="flex items-center justify-between">
                                ...
                                <button className="h-7 w-7 rounded-md flex items-center justify-center text-muted-foreground hover:text-foreground hover:bg-muted transition-smooth" onClick={() => setSheetOpen(false)}>
                                    <X className="h-4 w-4" />
                                </button>
                            </div>
                            ...
                            {/* Feature 6: Subtasks section */}
                            {editingTask && (
                                <div className="pt-4 border-t border-border/60">
                                    <SubtaskList parentId={editingTask.id} members={members} />
                                </div>
                            )}
                                    ...
                                    <div className="space-y-2.5 max-h-[250px] overflow-y-auto">
                                        {isLoadingActivities ? (
                                            ...
                                            <>
                                                ...
                                                {/* Feature 5: Load more activities */}
                                                {hasMoreActivities && (
                                                    <button
                    ...
                    <div className="flex gap-2 justify-end px-6 py-4 border-t border-border/60 bg-muted/20">
                        <Button variant="outline" size="sm" onClick={() => setSheetOpen(false)} className="h-9">Cancel</Button>
                        <Button type="submit" form="task-form" size="sm" className="h-9">{editingTask ? 'Save Changes' : 'Create Task'}</Button>
                    </div>
                </div>
            </Sheet>
        </div>
